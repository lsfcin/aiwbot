...
import time
from frontend.resume import _entry_line, _list_text, _header, _parse_arg, _meta, _keyboard

_NOW = time.time()


def test_meta_line_full_fields():
    item = {"backend": "claude", "updated_at": _NOW, "mode": "plan", "model": "claude-sonnet-5"}
    assert _meta(item) == "agora · plan · claude · sonnet"


def test_meta_line_omits_missing_mode_and_model():
    item = {"backend": "opencode", "updated_at": _NOW}
    assert _meta(item) == "agora · opencode"
